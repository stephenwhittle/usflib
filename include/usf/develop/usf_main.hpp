...
namespace usf {
  namespace internal {
    ...
    constexpr void process(std::span<CharT> &str, std::basic_string_view<CharT> &fmt,
                           const Argument<CharT> *const args, const int arg_count, Translatable locale = std_locale) {
  ...
  template <typename CharT, typename... Args>
  constexpr std::span<CharT> basic_format_to(std::span<CharT> str, Translatable locale, std::basic_string_view<CharT> fmt, Args &&...args) {
    // Nobody should be that crazy, still... it costs nothing to be sure!
    static_assert(sizeof...(Args) < 128, "usf::basic_format_to(): crazy number of arguments supplied!");
...
//  }
  ...
  template <typename... Args>
  constexpr std::span<char8_t> format_to(std::span<char8_t> str, Translatable locale, std::u8string_view fmt, Args &&...args) {
